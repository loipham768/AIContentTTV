...
export function LoginRegisterCard({ callbackUrl = '/editor', initialPlan = '' }: LoginRegisterCardProps) {
  ...
  function validateFields() {
    ...
    if (!emailSchema.safeParse(email).success) errors.email = 'Email không hợp lệ'
    if (!passwordSchema.safeParse(password).success) errors.password = 'Mật khẩu phải có ít nhất 8 ký tự'
  ...
  async function handleSendOtp() {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error || 'Gửi OTP thất bại'); return }
      setStep('otp')
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-12">
      ...
      <div className="relative z-10 flex flex-col items-center gap-6 w-full max-w-md">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-slate-500">Tạo khối nội dung HTML chuẩn từ mô tả tiếng Việt</p>
        </div>
          ...
          {step === 'otp' ? (
            <div className="space-y-5">
              ...
              <div className="flex flex-col items-center gap-3 text-center py-2">
                ...
                <div>
                  <p className="font-semibold text-slate-900">Nhập mã xác nhận</p>
                  <p className="text-sm text-slate-500 mt-0.5">
                    Mã 6 chữ số đã được gửi đến <span className="font-medium text-slate-700">{email}</span>
                  </p>
                </div>
              </div>
              ...
              <button onClick={handleVerifyOtp} disabled={loading || otp.length !== 6} className={submitBtn}>
            ...
            <>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onKeyDown={e => { if (e.key === 'Enter') tab === 'login' ? handleLogin() : handleSendOtp() }}
                ...
                >
                  {loading && <Loader2 className="w-4 h-4 animate-spin" />}
                  {loading ? 'Đang xử lý...' : tab === 'login' ? 'Đăng nhập' : 'Tiếp theo →'}
                </button>
