...
export default function HistoryPanel({
  ...
  function loadIntoEditor(editor: Editor, blockData: Record<string, unknown>) {
    ...
    if (blockData.type === "html" && typeof blockData.html === "string") {
      ...
      editor.loadProjectData(
        blockData as Parameters<typeof editor.loadProjectData>[0],
      );
    }
  }
  ...
  async function commitRename(projectId: string) {
    ...
    try {
      ...
      if (res.ok) {
        setProjects((prev) =>
          prev.map((p) => (p._id === projectId ? { ...p, name: trimmed } : p)),
        );
      } else {
  ...
  return (
    <div className="flex flex-col bg-slate-50 overflow-hidden w-full min-h-0">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && fetchError && (
          <div className="mx-3 my-4 p-3 bg-red-50 border border-red-100 rounded-xl flex items-start gap-2">
            ...
            <p className="text-xs text-red-500">
              Unable to load history. Please refresh the page.
            </p>
          </div>
        )}
        ...
        {!loading && !fetchError && projects.length === 0 && (
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Create your first content with AI below
            </p>
          </div>
        )}
        ...
        {!loading &&
          ...
          projects.map((project) => (
            <div
              key={project._id}
              className="mx-2 my-1.5 bg-white rounded-xl border border-slate-200 overflow-hidden hover:border-slate-300 hover:shadow-sm transition-all group"
            >
              <div className="px-3 pt-3 pb-2">
                <div className="mb-1">
                  {editingId === project._id ? (
                    ...
                    <div className="flex items-center gap-1 min-w-0">
                      <p
                        className="text-xs font-semibold text-slate-800 truncate leading-snug flex-1 min-w-0"
                        title={project.name}
                      >
                        {project.name}
                      </p>
      ...
      <ConfirmModal
        ...
        subText="Unsaved changes will be lost."
        confirmLabel="Replace"
        confirmVariant="primary"
        onConfirm={() => {
          if (modalState.type === "reopen") {
            handleConfirmReopen(modalState.project);
          }
        }}
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (modalState.type === "delete") {
            handleConfirmDelete(modalState.project);
          }
        }}
        onCancel={() => setModalState({ type: "none" })}
      />
