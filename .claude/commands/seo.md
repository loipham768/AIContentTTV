...
- Có ít nhất 2 internal link sang bài khác trong ARTICLES (dùng href="/kien-thuc/[slug-bài-khác]")
- CTA cuối bài luôn link về TaoPage
- Không dùng từ "chúng tôi" — dùng "TaoPage" hoặc "AITaoPage"
- Có số liệu thực tế (tỷ lệ %, thống kê) để tăng độ tin cậy

## Bước 4: Ghi JSON và upsert thẳng vào MongoDB

Tạo file `.claude/pending-article.json` với nội dung:
...
```json
{
  ...
  "content": "[HTML content — escape dấu ngoặc kép bên trong]"
}
```
...
```bash
pnpm add-article .claude/pending-article.json
```

Nếu thành công sẽ in: `✅ Inserted: [slug]` hoặc `✅ Updated: [slug]`. File JSON sẽ tự xóa sau khi chạy xong.

**KHÔNG viết vào `lib/articles.ts` nữa** — file đó chỉ còn type definition, không có data.
...
- Bài đã viết: [tiêu đề]
- Slug: [slug]
- URL: https://taopage.vn/kien-thuc/[slug]
