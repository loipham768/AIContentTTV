...
## Bước 4: Thêm vào lib/articles.ts

Thêm entry mới vào object `ARTICLES` trong `lib/articles.ts`. Cấu trúc:
...
Thêm vào TRƯỚC dấu `};` cuối cùng của object ARTICLES.

## Bước 5: Cập nhật sitemap (nếu cần)

Kiểm tra `app/sitemap.ts`. Nếu sitemap hardcode danh sách slug, thêm slug mới vào. Nếu sitemap đọc động từ ARTICLES thì bỏ qua bước này.
