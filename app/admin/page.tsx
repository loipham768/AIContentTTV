...
import { Users, LayoutTemplate, Activity, ShoppingCart, TrendingUp } from 'lucide-react'
...
async function getAdminData(sessionUserId: string) {
  ...
  const [totalUsers, totalProjects, activeUserIds, users, projectCounts, projects, orders, monthRevenue] = await Promise.all([
    User.countDocuments(),
    ...
    Order.aggregate([{ $match: { status: 'paid', activatedAt: { $gte: startOfMonth } } }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
  ])
  ...
  const orderRows: OrderRow[] = (orders as any[]).map(o => ({
    _id: o._id.toString(),
    orderId: o.orderId,
    userId: o.userId,
  ...
  const revenueThisMonth = (monthRevenue as any[])[0]?.total ?? 0

  return {
    stats: { totalUsers, totalProjects, activeUsers: activeUserIds.length, revenueThisMonth },
    userRows,
    projectRows,
    orderRows,
    meId: sessionUserId,
...
export default async function AdminPage() {
  ...
  const { stats, userRows, projectRows, orderRows, meId } = data
          ...
          {[
            ...
            <div key={label} className="bg-white rounded-2xl border border-gray-100 p-5 flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xl font-bold text-gray-900 truncate">{typeof value === 'number' ? value.toLocaleString('vi-VN') : value}</p>
