      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        ...
        {isPaid && (
          <div className="max-w-md mx-auto flex flex-col items-center text-center gap-6">
            ...
            <div>
              <p className="text-2xl font-bold text-emerald-800 mb-1">Cảm ơn bạn!</p>
              <p className="text-base text-emerald-700">Gói của bạn đã được kích hoạt thành công.</p>
            </div>
            ...
            <div className="w-full bg-white rounded-2xl border border-gray-100 shadow-sm p-5 text-sm text-left space-y-0">
              {[
                ...
                { label: 'Số tiền',    value: <span className="font-bold text-gray-900">{formatVnd(order.amount)}</span> },
                { label: 'Trạng thái', value: <span className="font-semibold text-emerald-600">Đã thanh toán</span> },
              ].map(({ label, value }) => (
            ...
            <Link href="/editor" className="w-full py-3 bg-emerald-600 text-white font-semibold rounded-xl hover:bg-emerald-700 transition-colors text-center">
        ...
        {isAwaitingConfirm && (
          <div className="max-w-md mx-auto flex flex-col items-center text-center gap-6">
            ...
            <div>
              <p className="text-2xl font-bold text-blue-800 mb-1">Đã nhận ảnh xác nhận!</p>
              <p className="text-base text-blue-700">
                Chúng tôi sẽ kiểm tra và kích hoạt gói trong vòng <strong>1–4 giờ</strong>.
              </p>
              <p className="mt-1 text-sm text-blue-500">Bạn sẽ nhận email thông báo khi gói được kích hoạt.</p>
            </div>
            ...
            <div className="w-full bg-white rounded-2xl border border-gray-100 shadow-sm p-5 text-sm text-left space-y-0">
              {[
                ...
                { label: 'Gói',        value: <span className="font-semibold text-gray-900">{orderTitle()}</span> },
                { label: 'Số tiền',    value: <span className="font-bold text-gray-900">{formatVnd(order.amount)}</span> },
                { label: 'Trạng thái', value: <span className="font-semibold text-blue-600">Chờ xác nhận</span> },
              ].map(({ label, value }) => (
                <div key={label} className="flex justify-between items-center py-2.5 border-b border-gray-50 last:border-0">
                  <span className="text-gray-500">{label}</span>
                  {value}
            ...
            <p className="text-xs text-gray-400">Bạn có thể đóng trang này và quay lại sau.</p>
          </div>
        )}
        ...
        {(isExpired || isCancelled) && (
          <div className="max-w-md mx-auto flex flex-col items-center text-center gap-6">
            ...
            <p className="text-xs text-gray-400">Đơn mới sẽ có hiệu lực trong 24 giờ kể từ khi tạo.</p>
          </div>
        )}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col">
              ...
              <div className="space-y-0 text-sm">
                {[
                  ...
                  { label: 'Trạng thái', value: <span className="font-semibold text-amber-600">Chờ thanh toán</span> },
                ].map(({ label, value }) => (
